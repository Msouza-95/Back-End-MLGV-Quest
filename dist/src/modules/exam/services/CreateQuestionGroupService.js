...
let CreateQuestionGroupService = class CreateQuestionGroupService {
    constructor(questionGroupRepository) {
        this.questionGroupRepository = questionGroupRepository;
    }
    async execute({ title, classs }) {
        const findGroup = await this.questionGroupRepository.findTitle(title);
        if (findGroup) {
            throw new AppError_1.default('QuestionGroup aready exists');
        }
